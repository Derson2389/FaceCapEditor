...
public class ARFaceTrackerNetCamera : MonoBehaviour
{
    public NetClient netClient;

    // 摄像机图像
    private UnityARSessionNativeInterface m_Session = null;
    ...
    void OnConnect()
    {
        Debug.Log("[ARFaceTrackerNetCamera.OnConnect] client connected to server");
    ...
    void OnDisConnect()
    {
        Debug.Log("[ARFaceTrackerNetCamera.OnDisConnect] client disconnected from server");
    ...
    void UpdateCamera(UnityARCamera camera)
    {
        if (!bTexturesInitialized)
        {
            ...
            NetMessage arCameraMessage = new NetMessage(NetMessageID.ARCameraMsgId);
            arCameraMessage.SetData(serARCamera.NetSerializeToByteArray());

            if (netClient != null && netClient.isConnected)
            {
                netClient.EnQueueMessage(arCameraMessage);
            }
        }
    ...
    public void OnPreRender()
    {
        ...
        NetMessage screenCaptureYMessage = new NetMessage(NetMessageID.ScreenCaptureYMsgId);
        SerializableByteData captureYBytesData = new SerializableByteData();
        captureYBytesData.data = YByteArrayForFrame(1 - currentFrameIndex);
        screenCaptureYMessage.SetData(captureYBytesData.NetSerializeToByteArray());
        netClient.EnQueueMessage(screenCaptureYMessage);

        /*
        NetMessage screenCaptureUVMessage = new NetMessage(NetMessageID.ScreenCaptureUVMsgId);
        SerializableByteData captureUVBytesData = new SerializableByteData();
        captureUVBytesData.data = UVByteArrayForFrame(1 - currentFrameIndex);
        screenCaptureUVMessage.SetData(captureUVBytesData.NetSerializeToByteArray());
        netClient.EnQueueMessage(screenCaptureUVMessage);
        */
    }
#endif
}

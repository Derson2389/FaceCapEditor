...
public class GateRecorderManager : RecorderManager
{
    ...
    public override void RenderConfigObjectItem(ConfigComponent config, GameObject selectobj, Rect rc)
    {
        ...
        else
        {
            ...
            {
                obj.MBNodeName = actorConfig.ActorMBName = MontionRecorderServer.Instance.MBObjectList[selectIndex].RootName;
            }
        }
        GUILayout.EndHorizontal();
    ...
    public void RefreshObjectList()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (!dummy.NeedRecord || !GateRecorderManager.Instance.IsConnected())
                continue;
    ...
    private void AddCloneObject(RecordObjInfo objInfo)
    {
...
#if UNITY_EDITOR
        int selectIndex = MontionRecorderServer.Instance.MBObjectList.FindIndex(c => c.RootName == objInfo.MBNodeName);
        if(selectIndex == -1)
        {
            return;
        }
        ...
        sync.Config(target, MontionRecorderServer.Instance.MBObjectList[selectIndex], objInfo.Root == objInfo.Target ? string.Empty : objInfo.Target.name, objInfo.SynDashIndex, objInfo.SynDashSekFile);

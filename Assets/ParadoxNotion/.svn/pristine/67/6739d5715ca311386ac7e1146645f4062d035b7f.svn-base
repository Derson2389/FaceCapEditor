...
namespace Slate{
	...
	abstract public class PropertiesTrack : CutsceneTrack, IKeyable {
		...
		/// add by TQ
        public Rect? DopePropertyRect = null;
       /// public bool IsDopeChoosed = false;
		/// end
		...
		public override void OnTrackTimelineGUI(Rect posRect, Rect timeRect, float cursorTime, System.Func<float, float> TimeToPos, Rect? selectRect = null)
        {
			...
			/// @modify slate sequencer
			/// add by TQ
			DopeSheetEditor.DrawDopeSheet(this.animationData, this, baseDopeRect, timeRect.x, timeRect.width ,true,DopeSheetStartPos, DopeSheedEndPos, MagnetSnapInterval);
			if (showCurves){
				var subDopeRect = Rect.MinMaxRect(posRect.xMin, posRect.yMin + defaultHeight, posRect.xMax, posRect.yMax);
				DoClipCurves(e, subDopeRect, timeRect, TimeToPos, this, posRect.x);
			}

            this.DopePropertyRect = posRect;
			/// end
        }

...
namespace Slate {
    ...
    public class Cutscene : MonoBehaviour, IDirector {
        ...
        // @hushuang
        // add this callback to notify StoryManager when will this cutscene will reach end time. 
        public event System.Action OnWillReachedEnd;
        ...
        private float _length = 600f;
        [SerializeField] [HideInInspector]
        private float _viewTimeMin = 0f;
        [SerializeField] [HideInInspector]
        private float _viewTimeMax = 21f;
        [SerializeField] [HideInInspector]
        private float _viewCurveBoxHeightMax = 0f;
        /// @modify slate sequencer
        /// add by TQ
        [SerializeField] [HideInInspector]
        ...
        [SerializeField]
        [HideInInspector]
        private int _currentRecordTime = 0;

        [SerializeField]
        [HideInInspector]
        private int _CutRecord = 1;

        [SerializeField]
        [HideInInspector]
        private float _snapInterval = 1f/30f;
    ...
    [System.NonSerialized]
...
#if UNITY_EDITOR
        private Object _prev_cutscene = null;
        public Object PrevCutscene
        {
            get { return _prev_cutscene; }
            set { _prev_cutscene = value; }
        }

        private bool _isSubEdit = false;
        public bool  IsSubEdite
        {
            get { return _isSubEdit;  }
            set { _isSubEdit = value; }
        }

        private Object _editFromCutClip = null;
        public Object EditFromCutClip
        {
            get { return _editFromCutClip;  }
            set { _editFromCutClip = value; }
        }
        ...
        public CameraTrack cameraTrack{
			get
            {
                return directorGroup.tracks.Find( t => (t is CameraTrack && t.isActive) ) as CameraTrack;
            }
		}
        // @end

        public CameraTrack cameraTrackCustom
        {
            get
            {
                return directorGroup.tracks.Find(t => (t is CameraTrack)) as CameraTrack;
            }
        }


        public CutsceneTrack GetTrackByName(string trackName)
        {
            return directorGroup.tracks.Find(t => (t.name == trackName)) as CutsceneTrack;       
        }


#if UNITY_EDITOR
        public void DeleteCameraTrack()
        {
            var trackDummy = directorGroup.tracks.Find(t => (t is CameraTrack && t.isActive)) as CameraTrack;
            directorGroup.DeleteTrack(trackDummy);
        }
#endif
...
/// add by TQ
        ...
        {
            get{ return _startPlayTime;}
            set{
                _startPlayTime = Mathf.Min(value, _length);
                _startPlayTime = Mathf.Max(0, _startPlayTime);
            }
        }
        ...
        {
            get { return _currentRecordTime;   }
            set { _currentRecordTime = value;  }
        }

        public int CutRecord
        {
            get { return _CutRecord;  }
            set { _CutRecord = value; }
        }
/// end
        //Min view time
        ...
        //Max Curve Box Height diff
        public float viewCurveBoxHeightMax
        {
            get { return _viewCurveBoxHeightMax;  }
            set
            {
                _viewCurveBoxHeightMax = Mathf.Min(value, 5000);
                _viewCurveBoxHeightMax = Mathf.Max(0, _viewCurveBoxHeightMax);
            }
        }
        ...
        public float playTimeStart{
			...
			set {
                _playTimeStart = Mathf.Clamp(value, 0, playTimeEnd);
            }
		}
        ...
        // @rongxia
        public bool EventSubCutscene = false;
        // @end
...
*/
			...
			if (!Application.isPlaying || currentTime > previousTime){
                for (var i = 0; i < timePointers.Count; i++) {
                    try
                    {
                        ...
                        bool DirectAudio = timePointers[i].Target is ActionClips.PlayAudio;
                        bool SubCutsence = timePointers[i].Target is ActionClips.SubCutscene;
                        if (DirectAudio || SubCutsence)
                        {
                            timePointers[i].TriggerForward(currentTime, previousTime);
                        }
                        else
                        {
                           /// currentTime = SnapTime(currentTime);
                            timePointers[i].TriggerForward(SnapTime(currentTime), previousTime);
                        }
			...
			if (!Application.isPlaying || currentTime < previousTime){
				for (var i = timePointers.Count-1; i >= 0; i--){
					try
                    {
                        ...
                        {
                            ///currentTime = SnapTime(currentTime);
                            timePointers[i].TriggerBackward(SnapTime(currentTime), previousTime);
                        }
			...
			if (unsortedStartTimePointers != null){
				for (var i = 0; i < unsortedStartTimePointers.Count; i++){
					try
                    {
                        ...
                        bool DirectAudio = unsortedStartTimePointers[i].Target is ActionClips.PlayAudio;
                        bool SubCutsence = unsortedStartTimePointers[i].Target is ActionClips.SubCutscene;
                        ...
                        {
                           /// currentTime = SnapTime(currentTime);
                            unsortedStartTimePointers[i].Update(SnapTime(currentTime), previousTime);
                        }
        ...
        // @modify slate sequencer
        // @rongxia
        public void DynamicAddTimePointers(IDirectable target)
        {
            var v = new StartTimePointer(target);
            timePointers.Add(new StartTimePointer(target));
            unsortedStartTimePointers.Add(v);
            timePointers.Add(new EndTimePointer(target));
            timePointers = timePointers.OrderBy(p => p.time).ToList();
        }
        // @end
...
/// @modify slate sequencer
/// add by TQ
            if (this.gameObject.GetComponent<ActionClips.SubCutscene>() != null)
            {
		...
		IEnumerator Internal_UpdateCutscene(){
			...
			while (isActive){
				...
				if (playingWrapMode == WrapMode.Once){
                    ...
                    // @modify slate sequencer
                    // @hushuang
                    // add OnWillReachedEnd callback to notify StoryManager when will this cutscene will reach end time. 
                    if (currentTime < playTimeEnd && currentTime + delta >= playTimeEnd && playingDirection == PlayingDirection.Forwards)
                    {
                        if (OnWillReachedEnd != null)
                            OnWillReachedEnd();
                    }

                    if (currentTime > playTimeStart && currentTime - delta <= playTimeStart && playingDirection == PlayingDirection.Backwards)
                    {
                        if (OnWillReachedEnd != null)
                            OnWillReachedEnd();
                    }
...
/// add by TQ
					...
					if (currentTime <= playTimeStart && temp){
...
/// end
...
#endif
        public bool WillShowCutsceneTime = false;
        public bool ShowCutsceneTime = false;
         // @modify slate sequencer
        // @rongxia
        ...
        private void OnGUI()
        {
            if(ShowCutsceneTime)
            {
                if(WillShowCutsceneTime || Cutscene.Current == this)
                {

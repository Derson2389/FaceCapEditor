...
namespace Slate{
	...
	public class AnimatedParameter : IAnimatableData{
		...
		public class SerializationMetaData{
            ...
            // @modify slate sequencer
            // @CYS
            public void injectiontraces( Stack<object> traces)
            {
                var temptraces = new Stack<object>(traces);
                temptraces = new Stack<object>(temptraces);
                
                //上一层靠近字符串开始的副本，最右侧是最外层
                while(temptraces.Count >0)
                {
                    var obj = temptraces.Pop();

                    if(obj is Component)
                    {
                        var t = (obj as Component).GetType();
                        var name = t.FullName;

                        var temp = string.Format("c:{0};", name);
                        this.traces = string.Format("{0}{1}", this.traces, temp);

                    }else if (obj is PropertyInfo)
                    {
                        var t = (obj as PropertyInfo);
                        var name = t.Name;

                        var temp = string.Format("p:{0};", name);
                        this.traces = string.Format("{0}{1}", this.traces, temp);

                    }else if(obj is FieldInfo)
                    {
                        var t = (obj as FieldInfo);
                        var name = t.Name;

                        var temp = string.Format("f:{0};", name);
                        this.traces = string.Format("{0}{1}", this.traces, temp);
                    }
        ...
        /// @modify slate sequencer
        /// @CYS
        public string traces { get { return data.traces; } }
        /// end
		///
        ...
        public AnimatedParameter(MemberInfo member, IKeyable keyable, Transform child, Transform root, Stack<object> traces)
        {
            this.keyable = keyable;
            if (member is PropertyInfo)
            {
                ConstructWithPropertyTraces((PropertyInfo)member, child, root, traces);
                return;
            }
            if (member is FieldInfo)
            {
                ConstructWithfieldTraces((FieldInfo)member, child, root, traces);
                return;
            }
            Debug.LogError("MemberInfo provided is neither Property nor Field");
        }
        ...
        void ConstructWithPropertyTraces(PropertyInfo targetProperty, Transform child, Transform root, Stack<object> traces)
        {
            if (!supportedTypes.Contains(targetProperty.PropertyType))
            {
                Debug.LogError(string.Format("Type '{0}' is not supported for animation", targetProperty.PropertyType));
            ...
            newData.parameterName = targetProperty.Name;
            newData.declaringTypeName = targetProperty.RTReflectedType().FullName;
            newData.injectiontraces(traces);
        ...
        void ConstructWithfieldTraces(FieldInfo targetField, Transform child, Transform root, Stack<object> traces)
        {
            if (!supportedTypes.Contains(targetField.FieldType))
            {
                Debug.LogError(string.Format("Type '{0}' is not supported for animation", targetField.FieldType));
			...
			if (targetObject is GameObject){
                ...
                if (!string.IsNullOrEmpty(traces))
                {
                    ...
                    if (!string.IsNullOrEmpty(transformHierarchyPath))
                    {
                        ...
                        baseobj = transform.gameObject;
                    }

                    var tracespath = traces.Split(';');
                    for (int i = tracespath.Length - 1; i >= 0; --i)
                    {
                        if (string.IsNullOrEmpty(tracespath[i]))
                        {
                            continue;
                        }
                        var tokens = tracespath[i].Split(':');


                        if (tokens.Length != 2)
                        {
                            Debug.Log("Error traces : " + tokens);
                        }

                        var typechar = tokens[0];
                        ...
                        switch (typechar)
                        {
                            case "c":
                                {
                                    baseobj = (baseobj as GameObject).GetComponent(typename);
                                }
                                break;
                            case "p":
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        var pinfo = basetype.GetProperty(typename);
                                        if (pinfo != null)
                                        {
                                            var a = pinfo.GetValue(baseobj, null);
                                            baseobj = a;
                                        }
                            ...
                            case "f":
                                {
                                    ...
                                    {
                                        var basetype = baseobj.GetType();
                                        var finfo = basetype.GetField(typename);
                                        if (finfo != null)
                                        {
                                            var a = finfo.GetValue(baseobj);
                                            baseobj = a;
                                        }
                                    }
                                }

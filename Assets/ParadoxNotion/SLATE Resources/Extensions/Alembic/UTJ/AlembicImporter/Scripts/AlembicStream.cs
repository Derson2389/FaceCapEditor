...
namespace UTJ.Alembic
{
    public class AlembicStream : AlembicDisposable
	{
        ...
        public void AbcLoad(bool createMissingNodes=false)
        {
            ...
            if (ImportSettings.m_pathToAbc.Contains("StreamingAssets"))
            {
                int idx = ImportSettings.m_pathToAbc.IndexOf("StreamingAssets");
                int length = "StreamingAssets".Length;
                string destPath = ImportSettings.m_pathToAbc.Substring(idx + length);
                ImportSettings.m_pathToAbc = destPath;
            }

            m_loaded = AbcAPI.aiLoad(m_abc, Application.streamingAssetsPath + ImportSettings.m_pathToAbc);
